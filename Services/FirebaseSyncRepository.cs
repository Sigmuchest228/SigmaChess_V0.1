...
// Репозиторий синхронизации с Firebase — весь обмен данными с облаком собран здесь.
// Отвечает за: профиль пользователя (создать/прочитать/обновить), сохранение
// завершённых партий и список id партий пользователя, загрузку кратких сводок сыгранных
// игр, список «уважения» (respects) и встречные записи, а также поиск пользователей по
// имени. Использует AppService для доступа к базе и текущему пользователю. Статические
// методы ToEndReason и ResolveWinnerColor переводят результат партии движка в строки для
// хранения.
