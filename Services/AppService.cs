...
// Центральный сервис приложения — «единая точка» для общих вещей. Реализован как
// синглтон (один экземпляр на всё приложение через GetInstance). Хранит подключение к
// Firebase (авторизация и база) и лениво создаёт общие объекты: репозиторий
// синхронизации, сервис раскладки доски, контроллер партии, главную игровую ViewModel,
// координатор нижнего меню, выбор фото. Также умеет вход/регистрацию/выход и хранит id
// текущего пользователя. К нему обращаются почти все ViewModel.
